...
class RelativeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        positions = torch.arange(T, device=x.device)
        rel_pos = positions.unsqueeze(1) - positions.unsqueeze(0) + (self.rel_emb.num_embeddings // 2)
...
class AFTLocal(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = (dist >= 0) & (dist < self.local_window)
        
        w = torch.zeros(T, T, device=x.device)
        w[mask] = self.w_local[dist[mask]]
...
class AFTConv(nn.Module):
    def __init__(self, embed_dim, context_len, kernel_size=3):
        super().__init__()
        self.q_proj = nn.Linear(embed_dim, embed_dim)
        self.k_proj = nn.Linear(embed_dim, embed_dim)
        self.v_proj = nn.Linear(embed_dim, embed_dim)
        self.conv = nn.Conv1d(embed_dim, embed_dim, kernel_size=kernel_size, groups=embed_dim, bias=False)
        self.proj = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, x):
        ...
        causal_padding = self.conv.kernel_size[0] - 1
        
        xc = self.ln(x).transpose(1, 2)
        xc = F.pad(xc, (causal_padding, 0)) 
        w_conv = self.conv(xc).transpose(1, 2)

        exp_k = torch.exp(k)
        num = torch.cumsum(exp_k * v, dim=1) * torch.exp(w_conv)
        den = torch.cumsum(exp_k, dim=1) * torch.exp(w_conv) + 1e-6
...
def train(model, train_data, val_data, config, save_path=None):
    ...
    for step in range(config["max_steps"]):
        ...
        optimizer.step()
        total_tokens += config["batch_size"] * config["context_len"]

        if step % config["eval_interval"] == 0:
            stats = estimate_loss(model, train_data, val_data, config)
    ...
    final_stats = estimate_loss(model, train_data, val_data, config)
    peak_mb = (
        torch.cuda.max_memory_allocated() / (1024 ** 2)
        if torch.cuda.is_available() else 0
    )
...
def test_extrapolation(model, val_data, context_len, device, steps=50):
    ...
    model.train()
    return np.exp(np.nanmean(losses)) if losses else float('nan')


def run_experiment(name, model, cfg, save_name):
    print(f"\n{name}")
    stats = train(model, train_data, val_data, cfg,
...
conv_combined_results = {}
test_config = config.copy()
test_config["context_len"] = 256  

print("\nTraining ConvSlidingRoPE (Hybrid Milestone)...")
conv_rope_model = InterleavedGPT(test_config, attn_fn=RoPESlidingWindowAttention, use_pos_emb=False).to(test_config["device"])
print(f"Params: {sum(p.numel() for p in conv_rope_model.parameters()):,}")

stats = train(conv_rope_model, train_data, val_data, test_config, 
              save_path="/kaggle/working/saidl/conv_rope_sw")

conv_combined_results["ConvSlidingRoPE"] = {
    "perplexity": stats["perplexity"],
    "peak_gpu_mb": stats["peak_gpu_mb"],
    "time_per_epoch_min": calc_epoch_time(
        stats["time_seconds"], 
        ...
        test_config["context_len"]
    ),
    "throughput": stats["throughput"]
}

print(f"\nHybrid Results: {conv_combined_results['ConvSlidingRoPE']}")